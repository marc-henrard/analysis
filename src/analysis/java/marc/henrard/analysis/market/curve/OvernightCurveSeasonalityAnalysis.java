...
public class OvernightCurveSeasonalityAnalysis {
  ...
  private static final ResourceLocator FILE_SETTINGS_ZRLIN =
      ResourceLocator.of(PATH_CONFIG + "USD-DSCONOIS-settings-zrlin.csv");
  private static final ResourceLocator FILE_NODES_FOMC =
  ...
  private static final ResourceLocator FILE_NODES =
      ResourceLocator.of(PATH_CONFIG + "USD-DSCONOIS-nodes.csv");
  ...
  private static final ImmutableRatesProvider MULTICURVE_PIECEWISE_CONSTANT_ON =
      MulticurveConfigDataSet.multicurve(VALUATION_DATE,
          GROUP_NAME,
          FILE_GROUP, FILE_SETTINGS_DFLL, FILE_NODES_FOMC, 
          FILE_QUOTES, REF_DATA);
  /* Export */
  private static final String PATH_LINEAR_ON = 
      "src/analysis/resources/output/overnight-zr-linear.csv";
  ...
  @Test
  public void linear_overnight_curve() throws IOException {
    ImmutableRatesProvider multicurveZrLinear =
        MulticurveConfigDataSet.multicurve(VALUATION_DATE,
            GROUP_NAME,
            FILE_GROUP, FILE_SETTINGS_ZRLIN, FILE_NODES, 
            FILE_QUOTES, REF_DATA);
    CurveExportUtils.exportOvernightCurve(
        multicurveZrLinear, USD_FED_FUND, 375, REF_DATA, "ON-FWD", PATH_LINEAR_ON);
  }
  ...
  public void step_overnight_seasonality_curve() throws IOException {
    ...
    RatesCurveGroupDefinition groupDefinitionLoad = RatesCalibrationCsvLoader
        .load(FILE_GROUP, FILE_SETTINGS_DFLL, FILE_NODES_FOMC).get(GROUP_NAME);
